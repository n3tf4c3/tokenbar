 ...
 */
export function renderDashboard(snapshot: UsageSnapshot, { interactive = true } = {}): string {
    const cards = snapshot.providers.map(provider => {
      const windows = provider.windows.map(window => {
        ...
        return `<article class="window">
          <div class="window-head"><span>${escapeHtml(window.label)}</span><strong class="${tone}">${window.usedPercent.toFixed(0)}% usado</strong></div>
          <div class="track"><div class="fill ${tone}" style="width:${window.usedPercent}%"></div></div>
          <div class="meta"><span>${formatReset(window.resetsAt)}</span></div>
