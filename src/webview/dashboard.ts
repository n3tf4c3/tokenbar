...
</style></head><body><main class="shell"><div class="top"><div><div class="brand">TokenBar</div><h1>Cotas das suas assinaturas</h1><div class="subtitle">Percentuais reais informados pelos serviços. “Restante” é a cota da assinatura, não o espaço de contexto da conversa.</div></div><button class="refresh" onclick="refresh()">↻ Atualizar agora</button></div><div class="grid">${cards}</div><div class="footer"><span>Atualização automática configurável</span><span>Coletado ${formatCollected(snapshot.collectedAt)}</span></div></main><script>const vscode=acquireVsCodeApi();function refresh(){vscode.postMessage({command:'refresh'})}</script></body></html>`;
  }
}
